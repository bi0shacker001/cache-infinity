...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--bootstrap",
        action="store_true",
        help="Load configuration from bootstrap.yml and write to database",
    )
    parser.add_argument(
        "--db-type",
...
def cmd_serve(args) -> None:
    ...
    # Load configuration using new two-file system
    try:
        from .core.config import load_two_file_settings, validate_settings
        settings = load_two_file_settings(config_dir, args, os.environ)
        ...
        # If --bootstrap flag is provided, write configuration to database
        if hasattr(args, 'bootstrap') and args.bootstrap:
            _LOGGER.info("Bootstrap mode enabled - writing configuration to database")
            # TODO: Implement database persistence for bootstrap configuration
            # This would involve writing the settings to the database so they can be read on subsequent runs
            _LOGGER.info("Bootstrap configuration written to database")
