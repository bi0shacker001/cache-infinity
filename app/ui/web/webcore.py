...
class WebUIApp:
    ...
    def __call__(self, environ, start_response):
        ...
        if path == "/api" or path.startswith("/api/"):
            _LOGGER.warning("API path requested on admin WebUI: %s", path)
            return self._json_error(start_response, "Unsupported path", status="404 Not Found")

        admin_exists = getattr(self.management, "users", None)
        if callable(admin_exists):
            has_admin_result = admin_exists("admin", "exists")
            has_admin = has_admin_result.get("exists", False)
        else:
            has_admin = False
        if not has_admin:
            _LOGGER.warning("No admin credentials found; showing login page for setup")
    ...
    def _handle_module_routes(self, path: str, method: str, environ, start_response):
        ...
        if path == "/storage" and method == "GET":
            ...
            return self._json_response(start_response, self.management.system("storage"))
        if path == "/shares" and method == "GET":
            _LOGGER.debug("Serving share list")
            return self._json_response(start_response, self.management.shares("list"))
        if path == "/downloads" and method == "GET":
            ...
            return self._json_response(
                start_response,
                self.management.downloads("list", statuses=statuses, limit=limit),
            )
        if path == "/rclone/remotes" and method == "GET":
            _LOGGER.debug("Serving rclone remotes list")
            try:
                return self._json_response(start_response, self.management.rclone("remotes"))
        ...
        if path == "/settings/detail" and method == "GET":
            ...
            try:
                ...
                settings_data = self.management.settings("detail")
        ...
        if path == "/settings/config" and method == "GET":
            ...
            return self._json_response(start_response, self.management.settings("config_get"))
        ...
        if path == "/cachelinks" and method == "GET":
            ...
            if 'cachelinks' in self.handlers:
                return self._json_response(start_response, self.management.cachelinks("tree"))
        ...
        if path == "/cookies" and method == "GET":
            ...
            if 'cookies' in self.handlers:
                return self._json_response(start_response, self.management.cookies("list"))
        ...
        if path == "/users" and method == "GET":
            ...
            if 'users' in self.handlers:
                return self._json_response(start_response, self.management.users("admin", "list"))
        ...
        if path == "/webdav-users" and method == "GET":
            ...
            if 'users' in self.handlers:
                return self._json_response(start_response, self.management.users("webdav", "list"))
            return self._json_error(start_response, "Users handler not available", status="500 Internal Server Error")
        ...
        if path == "/keys" and method == "GET":
            ...
            if 'users' in self.handlers:
                return self._json_response(start_response, self.management.api_keys("list"))
            return self._json_error(start_response, "Users handler not available", status="500 Internal Server Error")
        ...
        if path == "/reload" and method == "POST":
            ...
            try:
                ...
                result = self.management.system("reload", allow_switch=allow_switch, dump=dump)
                return self._json_response(start_response, result)
            except Exception as exc:
        ...
        if path == "/reinit" and method == "POST":
            ...
            try:
                result = self.management.system("reinit")
                return self._json_response(start_response, result)
            except Exception as exc:
                return self._json_error(start_response, str(exc), status="500 Internal Server Error")
        ...
        if path == "/degraded" and method == "GET":
            ...
            if 'maintenance' in self.handlers:
                return self._json_response(start_response, self.management.maintenance("degraded"))
            return self._json_error(start_response, "Maintenance handler not available", status="500 Internal Server Error")
    ...
    def _authenticate(self, environ) -> str | None:
        ...
        username = result.get("valid") if isinstance(result.get("valid"), str) else None
        if not username:
    ...
    def _handle_logout(self, environ, start_response):
        ...
        token = cookies.get("ci_session")
        if token:
            self.management.auth("logout", token=token)
        secure = ""
        if environ.get("wsgi.url_scheme") == "https" or environ.get("HTTP_X_FORWARDED_PROTO") == "https":
            secure = "; Secure"
        headers = [
    ...
    def _serve_status(self, start_response):
        ...
        try:
            _LOGGER.debug("DEBUG: About to call management.system('status')")
            data = self.management.system("status")
...
class OverviewHandlers:
    ...
    def handle_storage_utilization(self, environ, start_response):
        ...
        try:
            data = self.management.system("storage")
            return self._json_response(start_response, data)
        except Exception as exc:
            return self._json_error(start_response, str(exc), status="400 Bad Request")
    ...
    def handle_degraded_targets(self, environ, start_response):
        ...
        try:
            data = self.management.maintenance("degraded")
            return self._json_response(start_response, data)
        except Exception as exc:
            return self._json_error(start_response, str(exc), status="400 Bad Request")
...
class CachelinksHandlers:
    ...
    def handle_cachelink_create(self, payload: dict[str, object], start_response):
        ...
        try:
            snapshot = self.management.cachelinks(
                "create",
    ...
    def handle_cachelink_update(self, payload: dict[str, object], start_response):
        ...
        try:
            self.management.cachelinks("update", canonical_id=canonical_id, url=url, subfolder=subfolder, url_handler=url_handler)
    ...
    def handle_cachelink_preview(self, payload: dict[str, object], start_response):
        ...
        try:
            preview = self.management.cachelinks("preview", url=url, subfolder=subfolder, url_handler=url_handler)
    ...
    def handle_cachelink_folder_add(self, payload: dict[str, object], start_response):
        ...
        try:
            self.management.cachelinks("folder_add", path=path)
            return self._json_response(start_response, {"status": "ok"})
        except Exception as exc:
            return self._json_error(start_response, str(exc), status="400 Bad Request")
    ...
    def handle_cachelink_folder_delete(self, environ, start_response):
        ...
        try:
            self.management.cachelinks("folder_delete", path=folder_path)
            return self._json_response(start_response, {"status": "ok"})
        except Exception as exc:
            return self._json_error(start_response, str(exc), status="400 Bad Request")
    ...
    def handle_cachelink_delete(self, environ, start_response):
        ...
        try:
            self.management.cachelinks("delete", canonical_id=canonical_id)
            return self._json_response(start_response, {"status": "ok"})
        except Exception as exc:
            return self._json_error(start_response, str(exc), status="400 Bad Request")
...
class CookiesHandlers:
    ...
    def handle_cookie_domain_add(self, payload: dict[str, object], start_response):
        ...
        try:
            self.management.cookies("domain_add", domain=domain, cookie_jar=cookie_jar)
            return self._json_response(start_response, {"status": "ok"})
        except Exception as exc:
            return self._json_error(start_response, str(exc), status="400 Bad Request")
...
class MaintenanceHandlers:
    ...
    def handle_reindex(self, payload: dict[str, object], start_response):
        ...
        try:
            self.management.maintenance("reindex", canonical_id=canonical_id)
...
class SettingsHandlers:
    ...
    def handle_settings_detail_update(self, payload: dict[str, object], start_response):
        ...
        try:
            self.management.settings("update", payload=payload)
...
class UsersHandlers:
    ...
    def handle_user_disable(self, environ, start_response):
        ...
        try:
            # For admin users, we need to disable them via the manage action
            self.management.users("admin", "manage", username=username, enabled=False)
            return self._json_response(start_response, {"status": "ok"})
        except Exception as exc:
            return self._json_error(start_response, str(exc), status="400 Bad Request")
    ...
    def handle_webdav_user_delete(self, environ, start_response):
        ...
        try:
            ...
            # For WebDAV users, we need to disable them via the manage action with enabled=False
            self.management.users(
                "webdav",
                "manage",
                username=username,
    ...
    def handle_api_key_generate(self, payload: dict[str, object], start_response):
        ...
        try:
            result = self.management.api_keys("generate", username=username)
            return self._json_response(start_response, result)
        except Exception as exc:
            return self._json_error(start_response, str(exc), status="400 Bad Request")
    ...
    def handle_api_key_revoke(self, environ, start_response):
        ...
        if not username:
            return self._json_error(start_response, "username required", status="400 Bad Request")
        try:
            self.management.api_keys("revoke", username=unquote(username))
            return self._json_response(start_response, {"status": "ok"})
        except Exception as exc:
            return self._json_error(start_response, str(exc), status="400 Bad Request")
